...
Agents typically access data through tools—whether retrieving documents from vector databases, querying SQL tables, or calling APIs. Each pattern differs in how data is stored, indexed, and retrieved. The following patterns represent the main approaches for integrating proprietary data into agent systems.
...
#### 2. Database and API Access
Database and API access patterns address the need for real-time data and transactional operations that require guaranteed freshness and consistency. Instead of pre-indexing data into vector stores, agents query live systems directly through specialized tools. This includes executing SQL queries against operational databases, calling internal APIs for real-time data, and parsing structured documents on demand. While this approach ensures data freshness, it requires careful management of query permissions, rate limits, and response times.
