...
**Monitoring & alerts** shift from basic health metrics to detecting complex failure modes. Simple systems monitor behavior drift and rate limits, RAG systems add retrieval quality metrics, tool-calling agents track tool execution health, and multi-agent systems must detect emergent problems like infinite loops and escalation bottlenecks.
...
Manual testing catches obvious errors but misses edge cases and doesn't scale. Production agents encounter diverse inputs that expose failure modes invisible during development. Build evaluation suites and continuously improve them.
