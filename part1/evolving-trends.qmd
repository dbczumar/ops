...
**AgentOps (2024+)**: The practice and tooling to build, evaluate, deploy, govern, monitor, and maintain autonomous AI systems that can reason, plan, and take actions. Unlike LLMs that operate in single request-response cycles, agents execute multi-step workflows using tools to interact with real systems—querying databases, calling APIs, modifying files, and executing code. This shift from single-step inference to active tool use introduced entirely new operational challenges: establishing permission boundaries for tools, tracing complex multi-step reasoning chains, preventing cascading failures when agents make mistakes, handling retry logic and error recovery, and orchestrating multiple agents without conflicts or infinite loops. Databricks addresses these challenges through Agent Bricks, a managed platform that automatically builds, optimizes, and deploys AI agents—handling model selection, fine-tuning with proprietary data, evaluation, monitoring, and continuous optimization behind the scenes. For teams that need more control, Databricks also provides capabilities for DIY, code-first agent development: evaluation frameworks to test agent behavior before deployment, comprehensive tracing for debugging multi-step decisions, secure tool execution with permission controls, monitoring for cost and performance, and serving infrastructure that scales to zero when idle.

### Why AgentOps Is Necessary

The jump from LLMs to agents introduced risks and complexities that existing practices couldn't handle:

**Agents can cause real damage**. When an LLM hallucinates, you get wrong text. When an agent hallucinates, it might delete production data or send emails to your entire customer base. The stakes are fundamentally different.

**Failures cascade in complex ways**. A simple prompt error can trigger a chain reaction: the agent misinterprets the task, calls the wrong API, gets unexpected data back, makes a bad decision based on that data, and continues down an increasingly wrong path. Traditional monitoring can't track these multi-step failure modes.

**Debugging requires specialized tooling**. When an agent fails after 15 steps across 5 different tools, you need to understand not just what happened, but why the agent made each decision. Standard logs and metrics don't capture the reasoning chains and decision points.

**Multiple agents create coordination challenges**. When you have several agents working together, you face new problems: agents getting stuck in loops, conflicting actions, resource contention, and emergent behaviors that no single agent exhibits alone.

**Safety boundaries must be enforced at runtime**. You can't just filter an agent's final output—you need to prevent it from taking dangerous actions in the first place. This requires real-time governance and permission systems that traditional ML pipelines never needed.

### Core AgentOps Principles

Building on lessons from MLOps and LLMOps, AgentOps establishes five core principles:

1. **Reliability**: Agents must perform consistently across diverse inputs and edge cases
2. **Observability**: Every decision point and tool call must be traceable and explainable  
3. **Safety**: Hard boundaries prevent agents from taking harmful or unintended actions
4. **Scalability**: Systems must support multiple concurrent agents without resource conflicts
5. **Maintainability**: Agent behaviors must be updatable without breaking existing workflows
