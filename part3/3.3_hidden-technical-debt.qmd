...
- PRs re-implement existing tools and connectors. There is an unclear “source of truth”
...
- Maintain a tool registry (owner, purpose, version, examples, scope). As an option, use MCP for typed schemas, discovery, and central policy to deduplicate tool connectors and reduce wrong-tool calls
...
Even without code changes, agent behavior shifts as input, prompts, tools, or the underlying model evolve. Quality decays, routes change, and budgets are breached without an obvious “event.”

**How to identify**

- Online LLM-as-judge scores on live samples trend down for task success, groundedness vs retrieved context, correct tool choice/parameters, and side-effect safety
...
- LLM-judge monitoring with alerts on sustained drops or threshold breaches
- Alert on quality, cost, and latency. Auto-roll back on breach
- Canary/shadow new versions. Replay historical traces before promotion
- Refresh prompts or update tools/routing based on measured behavior drift
- Calibrate judges: maintain a small human-labeled audit set; check judge agreement; version the judge's prompt and model
...
- Lower accuracy and trust, rising retries and cost, longer latency from repeated retrieval
...
flowchart TD
    ...
    SchemaChange --> ToolConnectorRetries
    ToolConnectorRetries --> QueueBacklog
...
- Agent behaves differently in staging vs prod despite identical code (trace tags show mismatched versions or configurations)

**Impact**

- Higher latency and cost, more incident volume, slower recovery
- Hard onboarding: slow, error-prone ramp-up for new contributors due to many moving parts, insufficient documentation, and unclear ownership
- Brittle deployments
...
- Use infrastructure as code with staged deploys and one-click rollback
- Schedule periodic “debt sprints” to remove dead code, unused tools, and redundant queues
...
- Frequent rollbacks due to quality regressions

**Impact**

- Stagnant quality 
- Low trust from domain users
- The agent works, but it doesn’t solve the user’s real job or fit their workflow, so adoption and satisfaction stay low
- The same failure patterns keep coming back because feedback isn’t captured into tests or prompts and nothing blocks regressions
- Difficulty prioritizing fixes
- Reduced delivery velocity as rollbacks become common
